...
def load_images(path: str, file_ending: str=".png") -> (list, int, int):
    ...
    Args:
        path (str): path of directory containing image files 
                    that can be assumed to have all the same dimensions
        file_ending (str): string specifying what image files have to end with, 
    ...
    for cur_file in files:
        if not cur_file.endswith(file_ending):
            continue

        try:
            image = mpl.image.imread(path + cur_file)
...
if __name__ == '__main__':
    ...
    for i, test_image in enumerate(images_test):
        ...
        image = test_image.flatten().reshape(1, -1) # 1*p
        image -= means # 1*p
        ...
        coeff_test_image = np.dot(image, eigenbasis.T)
        print("encoded / compact image shape: ", coeff_test_image.shape)

        # reconstruct from coefficient vector and add mean
        reconstructed_image = np.dot(coeff_test_image, eigenbasis)
        reconstructed_image += means
    ...
    plt.savefig(f"pca_solution-{k}.png")
    plt.show()
